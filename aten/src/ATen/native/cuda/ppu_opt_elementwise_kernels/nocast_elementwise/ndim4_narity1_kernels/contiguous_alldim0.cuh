...
// Covers the d1<->d2 transpose copy forms (groups 27-30) without a shared-
// memory tile transpose: IN is contiguous on dim0 and its higher-dim strides
// are arbitrary multiples of the vector width, so a plain vectorized load/
// store pair along dim0 is correct and coalesced for every such layout.

template <int nt, int vt, typename res_t, typename arg0_t, typename func_t>
C10_LAUNCH_BOUNDS_2(nt, 4)
__global__ void elementwise_kernel_4_1_contiguous_alldim0(
    int64_t N,
    ...
    func_t f) {
  // ndim = 4, arity = 1. Output and input contiguous on dim0; their
  // higher-dim strides are arbitrary multiples of the vector width (0 for a
  // broadcast dim), so row gaps (sliced views) are fine for the vectorized
  // load/store pair along dim0.
...
template <int nt, int vt, typename res_t, typename arg0_t, typename func_t>
static void launch_elementwise_kernel_4_1_contiguous_alldim0(
    int64_t N,
    char* data0, char* data1,
    int64_t size0, int64_t size1, int64_t size2, int64_t size3,
    int64_t stride00, int64_t stride01,
  ...
  elementwise_kernel_4_1_contiguous_alldim0<nt, vt, res_t, arg0_t, func_t>
      <<<grid, block, 0, stream>>>(
          N, data0, data1, size0, size1, size2, size3,
