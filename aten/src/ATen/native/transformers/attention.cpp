...
inline std::string print_tensor_info(const Tensor& t, std::string msg)
{
    ...
    // std::cout << "], dtype:" << t.dtype() << ", requires_grad:" << t.requires_grad() << ", ";
...
inline void print_sdpa_params(
  ...
  if (pEnv_perf) {
      ...
      // std::cout << ", dropout:" << dropout_p << ", is_causal:" << is_causal << ", scale:" << scale.has_value() << std::endl;
...
Tensor scaled_dot_product_attention(
  ...
  if (backend == SDPBackend::flex_flash_attention) {
    ...
    attn_mask = convert_attn_func(attn_mask_, query_.dtype());
  }
#else
  const auto convert_attn_func = backend != SDPBackend::cudnn_attention ? convert_boolean_attn_mask : convert_boolean_attn_mask_cudnn;
